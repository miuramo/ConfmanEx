...
<div class="px-4">
    ...
    <div class="py-2">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 dark:text-gray-300">
            ...
            LOG_LEVEL: {{ env('LOG_LEVEL', 'info') }}<br>
        </div>
    </div>

    <x-element.h1>問題のあるメールアドレスの管理
        <span class="mx-6"></span>
        <x-element.linkbutton color="orange" href="{{ route('contact.index') }}" >
            コンタクト管理
        </x-element.linkbutton>
    </x-element.h1>
    ...
    <x-element.h1>Danger Zone：
        ...
        <span class="px-5"></span>
        <x-element.linkbutton href="{{ route('enq.resetenqans_nopaper') }}" color="blue">
            アンケート・参加登録回答の選択的削除（削除済み論文のみを対象）
        </x-element.linkbutton>
        <span class="px-5"></span>
