    ...
    <div class="mx-6">
        ...
        @if ($ticket)
            ...
            <div class="mx-4">
                ...
                <p>投票トークンをお送りしたメールアドレス： {{ $ticket->email }}</p>
                @if ($ticket->user_id)
                    <p class="text-blue-500">投票トークンは現在ログイン中のユーザアカウントに紐づいています。どのブラウザでも「ログイン」→「投票」で投票できます。</p>
                @else
                    <p class="text-purple-500">投票トークンはユーザアカウントに紐づいていません。このブラウザ以外で投票を行う場合は、メールに記載されたURLを再度開く必要があります。</p>
                @endif
            </div>
        @endif
