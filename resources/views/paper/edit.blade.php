                ...
                <div class="m-6">
                    <x-element.h1>
                        ...
                        <div class="text-md mx-6 mt-0 dark:text-gray-500">
                            ...
                            @php
                                $nameofauthorlist = [];
                                $nameofauthorlist[] = $koumoku['authorlist'] ?? null;
                                $nameofauthorlist[] = $koumoku['eauthorlist'] ?? null;
                                $nameofauthorlist = array_filter($nameofauthorlist);
                                $nameofauthorlist = implode('、', $nameofauthorlist);
                            @endphp
                            <div class="mt-2 text-sm px-2  dark:text-gray-400">
                                注：
                                {{ $nameofauthorlist }}の書き方は、以下の例に合わせてください。詳細は
                                [書誌情報の設定]→[著者名の設定方法を表示] を参照してください。</div>
                            <div class="grid gap-6 lg:grid-cols-2 lg:gap-1">
                                @isset($koumoku['authorlist'])
