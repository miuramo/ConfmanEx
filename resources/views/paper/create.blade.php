        ...
        <div class="py-2 px-6">
            <form action="{{ route('paper.store') }}" method="post" id="newpaper">
                ...
                <div class="m-6">
                    ...
                    <x-element.h1>
                        「投稿連絡用メールアドレス」を入力してください。（<b>1件は必須</b>、最大{{ env('CONTACTEMAILS_MAX', 5) }}件まで、1行に1件ずつ）
                        ...
                        @if ($anyopen)
                            ...
                            @foreach ($cats as $c)
                                ...
                                @elseif($c->isOpen() && !$c->isnotUpperLimit())
                                    <div class="bg-slate-50 p-2 text-pink-600">
                                        申し訳ありませんが、{{ $c->name }}の投稿可能枠数は、現在残り0枠となっています。</div>
                                @endif
                                &nbsp;
                            @endforeach
                        @else
